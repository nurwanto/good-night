...
RSpec.describe Api::V1::BedTimeController, type: :controller do
  ...
  let(:following_user) { User.create!(name: 'Bob') }
  let!(:sleep_histories) do
    [
      BedTimeHistory.create!(user: following_user, bed_time: 10.hours.ago, wake_up_time: 2.hours.ago), # 8 hours
      BedTimeHistory.create!(user: following_user, bed_time: 12.hours.ago, wake_up_time: 6.hours.ago), # 6 hours
      BedTimeHistory.create!(user: following_user, bed_time: 13.hours.ago, wake_up_time: 6.hours.ago)  # 7 hours
    ]
  end
  ...
  describe 'GET #history' do
    context 'when fetching the first page' do
      it 'returns the correct data and meta information' do
        ...
        expect(json_response['data'].first['id']).to eq(sleep_histories.first.id)
        expect(json_response['data'].last['id']).to eq(sleep_histories.last.id)


        expect(json_response['meta']['next_cursor']).to eq("#{sleep_histories.last.sleep_duration}-#{sleep_histories.last.id}")
    ...
    context 'when fetching the second page using a cursor' do
      it 'returns the remaining data and meta information' do
        cursor = "#{sleep_histories.last.sleep_duration}-#{sleep_histories.last.id}"
        ...
        expect(json_response['data'].first['id']).to eq(sleep_histories[1].id)
  ...
  describe 'POST #set_unset' do
    context 'when setting bed_time' do
      it 'creates a new bed_time record if the user has woken up' do
        ...
        expect(current_user.sleep_histories.last.bed_time).to be_within(1.second).of(Time.now)
      end
    ...
    context 'when setting wake_up' do
      it 'updates the last bed_time record with wake_up_time if the user has slept' do
        ...
        expect(current_user.sleep_histories.last.wake_up_time).to be_within(1.second).of(Time.now)
      end
